...
df_data2 = pd.read_excel("Budget.xlsx", sheet_name="Suivi budget")
#df_cop=df_data2[['High','Low','Open','Close']].copy()
layout=html.Div(children=[
    dbc.Container([
        dbc.Row([
            html.P(' .', style={'margin-bottom': '0.25em'}),

            dbc.Col([html.H1('Savings', style={'font-size': '350%',
                                                 'text-align': 'center',
                                                 'color':'#A1C298',
                                                 'font-weight': 'heavy',
                                                 'margin-top': '2em',
       ...
       dbc.Row([html.Img(src="assets/income.png", style={"max-width": "10%",'margin-bottom': '1.5em','text-align':'center'})],justify='center') ,
        dbc.Row([
                    dbc.Col([
                        dcc.DatePickerRange(
                            id='date-pick-S',
                            min_date_allowed=df_data2.date.min(),
                            max_date_allowed=df_data2.date.max(),
                            start_date=df_data2.date.min(),
                            end_date=df_data2.date.max(),
        ...
        dbc.Row([
            dbc.Col([
                dcc.Loading(dcc.Graph(id='Bar_Chart_S'),color='red',type='cube')
            ])
        ],style=Style_Row2),
